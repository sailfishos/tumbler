...
static void
on_queue_answered_destroy (gpointer user_data)
{
  SpecializedInfo *sinfo = user_data;

  g_object_unref (sinfo->proxy);
}

static void
on_queue_answered_cb (DBusGProxy     *proxy,
                      DBusGProxyCall *call,
                      gpointer        user_data)
{
  SpecializedInfo *sinfo = user_data;

  dbus_g_proxy_end_call (proxy, call, &(sinfo->error),
                         G_TYPE_UINT, &(sinfo->handle),
                         G_TYPE_INVALID);

  g_mutex_lock (sinfo->mutex);
  g_cond_broadcast (sinfo->condition);
  sinfo->had_callback = TRUE;
  g_mutex_unlock (sinfo->mutex);
}

static gboolean
to_mainloop_start_cb (gpointer user_data)
{
  SpecializedInfo *sinfo = user_data;

  dbus_g_proxy_connect_signal (sinfo->proxy, "Finished",
                               G_CALLBACK (specialized_finished),
                               sinfo,
                               NULL);

  dbus_g_proxy_connect_signal (sinfo->proxy, "Ready",
                               G_CALLBACK (specialized_ready),
                               sinfo,
                               NULL);

  dbus_g_proxy_connect_signal (sinfo->proxy, "Error",
                               G_CALLBACK (specialized_error),
                               sinfo,
                               NULL);

  dbus_g_proxy_begin_call (sinfo->proxy, "Queue", 
                           on_queue_answered_cb,
                           sinfo,
                           on_queue_answered_destroy,
                           G_TYPE_STRING, sinfo->uri,
                           G_TYPE_STRING, sinfo->mime_type,
                           G_TYPE_STRING, sinfo->flavor_name,
...
static gboolean
to_mainloop_stop_cb (gpointer user_data)
{
  SpecializedInfo *sinfo = user_data;

  dbus_g_proxy_disconnect_signal (sinfo->proxy, "Finished",
                                  G_CALLBACK (specialized_finished),
                                  sinfo);

  dbus_g_proxy_disconnect_signal (sinfo->proxy, "Ready",
                                  G_CALLBACK (specialized_ready),
                                  sinfo);

  dbus_g_proxy_disconnect_signal (sinfo->proxy, "Error",
                                  G_CALLBACK (specialized_error),
                                  sinfo);
                                        ...
                                        TumblerFileInfo    *info)
{
  ...
  sinfo.flavor_name = flavor_name;
  sinfo.proxy = g_object_ref (s->proxy);
  sinfo.error = NULL;

  g_idle_add (to_mainloop_start_cb, &sinfo);

  g_mutex_lock (sinfo.mutex);
   if (!sinfo.had_callback)
     {
       g_cond_wait (sinfo.condition, sinfo.mutex);
     }
  g_mutex_unlock (sinfo.mutex);
  ...
  if (sinfo.error == NULL)
  ...
  else
    {
      ...
      g_signal_emit_by_name (thumbnailer, "error", uri, 1, message);
      g_free (message);
      g_clear_error (&sinfo.error);
    }

  sinfo.had_callback = FALSE;
  sinfo.proxy = g_object_ref (s->proxy);

  g_idle_add (to_mainloop_stop_cb, &sinfo);
