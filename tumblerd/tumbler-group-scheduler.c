  ...
  for (lp = g_list_last (missing_uris); lp != NULL; lp = lp->prev)
    {
      ...
      error_id = g_signal_connect (request->thumbnailers[n], "error", 
                                   G_CALLBACK (tumbler_group_scheduler_thumbnailer_error),
                                   &uri_errors);

      /* connect to the ready signal of the thumbnailer */
      ready_id = g_signal_connect (request->thumbnailers[n], "ready",
      ...
      g_signal_handler_disconnect (request->thumbnailers[n], error_id);
      g_signal_handler_disconnect (request->thumbnailers[n], ready_id);
